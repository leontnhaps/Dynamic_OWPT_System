...
def project(point_world, angles_deg, focal, cfg):
    ...
    if z <= 0:
        raise ValueError("Target behind camera: outside local simulator assumptions")
...
class TrackingEnv(gym.Env):
    ...
    def reset(self, *, seed=None, options=None):
        ...
        self.scenario = (options or {}).get("scenario", str(trajectory_rng.choice(["sine", "linear"])))
        if self.scenario not in ("sine", "linear", "stationary"):
            raise ValueError("scenario must be sine, linear or stationary")
    ...
    def step(self, action):
        ...
        reward_error = error if visible else uv-[c.laser_u, c.laser_v]
